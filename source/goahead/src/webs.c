...
#endif
#if 0
			if(wp->flags & WEBS_GOFORM_FIRMWARE_UPLOAD)
			{
				if (fd == -1) {
					fd = gopen(wp->cgiStdin, O_CREAT | O_WRONLY | O_BINARY  | O_APPEND,	0666);
				}
 ...
 */
	...
	if (gstrstr(url, CGI_BIN) != NULL) {
		...
		if(gstrstr(url, CGI_FIRMWARE_UPLOAD) != NULL){
			extern int g_cameraRegisterFlag;
			//g_cameraRegisterFlag = WEBPAGE_QUIT_STATUS_UPGRADEFIRMWARE;
			wp->flags |= WEBS_CGI_FIRMWARE_UPLOAD;
			printf("YYYYYYYYY WEBS_CGI_FIRMWARE_UPLOAD YYYYYYY");
		}

		if(gstrstr(url, CGI_ROOTFS_UPLOAD) != NULL){
			extern int g_cameraRegisterFlag;
			//g_cameraRegisterFlag = WEBPAGE_QUIT_STATUS_UPGRADEFIRMWARE;
			wp->flags |= WEBS_CGI_ROOTFS_UPLOAD;
			printf("YYYYYYYYY WEBS_CGI_ROOTFS_UPLOAD YYYYYYY");
		}

		if(gstrstr(url, CGI_LINUX_UPLOAD) != NULL){
			extern int g_cameraRegisterFlag;
			//g_cameraRegisterFlag = WEBPAGE_QUIT_STATUS_UPGRADEFIRMWARE;
			wp->flags |= WEBS_CGI_LINUX_UPLOAD;
			printf("YYYYYYYYY WEBS_CGI_LINUX_UPLOAD YYYYYYY");
		}

		if(gstrstr(url, CGI_ALL_UPLOAD) != NULL){
			extern int g_cameraRegisterFlag;
			//g_cameraRegisterFlag = WEBPAGE_QUIT_STATUS_UPGRADEFIRMWARE;
			wp->flags |= WEBS_CGI_ALL_UPLOAD;
			printf("YYYYYYYYY WEBS_CGI_ALL_UPLOAD YYYYYYY");
		}
		
		if (wp->flags & WEBS_POST_REQUEST) {
			wp->cgiStdin = websGetCgiCommName();
			printf("1. wp->cgiStdin=%s\n", wp->cgiStdin);
		}
		// ex: upload_setting....
		if(gstrstr(url, CGI_UPLOAD) != NULL){
			system("upload_kill.sh");
			wp->flags |= WEBS_CGI_UPLOAD;
			printf("HHHHHHH UPLOAD SETTING HHHHHH \n");
		}
	}
#if 0
	else if(gstrstr(url, WEBS_FIRMWARE_UPLOAD) != NULL)
	{
		printf("YYYYYYYYY WEBS_GOFORM_FIRMWARE_UPLOAD YYYYYYY\n");
		wp->flags |= WEBS_GOFORM_FIRMWARE_UPLOAD;
		if (wp->flags & WEBS_POST_REQUEST) {
			wp->cgiStdin = websGetCgiCommName();
			printf("2. wp->cgiStdin=%s\n", wp->cgiStdin);
		}
	}
#endif
#endif
