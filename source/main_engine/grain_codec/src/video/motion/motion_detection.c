...
extern EventCallback	mEventCallback ;  //事件回调
        ...
        else
        {
            ...
            if (block_count * 100 > block_num * (100 - alarm_th))
            {
				...
				if(mEventCallback != NULL)
				{
					...
					if((event.event_time - oldtime) > 15)//	 十五秒内
					{
...
#else
				if((event.event_time - oldtime) > 1)//	 十五秒内
				{
					oldtime = event.event_time;
					usleep(300*1000);
					...
					else if(1 == md_mode)
					{
					 ...
					 if(0== md_ir_flag)
					 {
					 	mEventCallback((void *)(&event));//发出报警
						warning("Motion Event pull\n");	
					 }
					 else
					 {
...
int motion_detection_update(void *bindfd,  mdt_alg_t *mdt_alg)
{
    ...
    if (ret < 0)
    {
        printf("MD_Test: capture_motion_info_init error at mdt_idx(%d), bindfd(0x%p)\n", mdt_idx,
               bindfd);
        capture_motion_end(mdt_idx); //motion detectione end 移动侦测结束
        ret = -1;
        goto err_ext;
    }
