...
int adapt_param_recovery_param(sdk_default_param_t *p_default, uint32_t ch_def)
{
	...
	if (PARAM_MASK_ALL == param_mask)		// 恢复整机参数
	{
		...
		adapt_param_get_def_sys_cfg(&sys_cfg);
		adapt_param_set_sys_cfg(&sys_cfg);

		//百度信息、
		sdk_baidu_info_t  baidu_cfg;
		memset(&baidu_cfg,0,sizeof(sdk_baidu_info_t));
		adapt_param_get_def_baidu_cfg(&baidu_cfg);
		adapt_param_set_baidu_cfg(&baidu_cfg);

		//wifi 参数
		sdk_wifi_cfg_t  wifi_cfg;
		memset(&wifi_cfg,0,sizeof(sdk_wifi_cfg_t));
	  	 adapt_param_get_def_wifi_cfg(&wifi_cfg);
	  	 adapt_param_set_wifi_cfg(&wifi_cfg);

		//网络参数
		sdk_eth_cfg_t  eth_cfg;
	   memset(&eth_cfg,0,sizeof(sdk_eth_cfg_t));
       adapt_param_get_def_eth_cfg(&eth_cfg);
...
/* ----------------------------------------------- */
#define DHCP_DEFAULT_SCRIPT	"/etc/dhcp/default.script"
//#define DHCP_DEFAULT_SCRIPT	"/usr/share/udhcpc/default.script"
...
int adapt_os_start_dhcp(char *p_net_name)
{
    ...
    //system(szStr);
#if 1   
	chmod("/sbin/udhcpc", 0777);
	sprintf(szStr, "/sbin/udhcpc -i %s -b -s %s &", p_net_name, DHCP_DEFAULT_SCRIPT);
...
int adapt_os_get_ip_info(sdk_ip_info_t *ip_info, int idx)
{
	...
	//ret = os_get_gateway1(if_name, (char *)ip_info->gateway);
...
int adapt_param_set_timezone_cfg(sdk_date_t *time_cfg)
{
    CHECK_NET_IDX(0);
    SDK_PARAM_SET_(0, SDK_MAIN_MSG_SYSTIME, time_cfg);
    return 0;
}
